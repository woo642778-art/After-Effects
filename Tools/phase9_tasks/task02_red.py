...
# Add only the test target first. It intentionally cannot import VertexTimeline yet.
for package_path in ["Package.swift", "Package@swift-6.0.swift"]:
    text = read(package_path)
    if 'name: "VertexTimelineTests"' in text:
        continue
    if package_path == "Package.swift":
        ...
        marker = '    .testTarget(name: "VertexProjectTests", dependencies: ["VertexProject", "VertexCore", "VertexMedia", "VertexRender"]),'
        insert = marker + '\n    .testTarget(name: "VertexTimelineTests", dependencies: ["VertexProject", "VertexCore"]),'
    if marker not in text:
        raise RuntimeError(f"Package marker not found: {package_path}")
    write(package_path, text.replace(marker, insert, 1))

write("Tests/VertexTimelineTests/TimelineEngineTests.swift", r'''import Testing
import VertexCore
...
print("Task 2 RED tests applied")
