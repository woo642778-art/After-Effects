...
func preCorrectionSchema2ImportPreservesLayerGraph() throws {
    ...
    let composition = ProjectComposition(
        ...
        width: 1920,
        height: 1080,
        duration: RationalTime(value: 8, timescale: 1),
        frameRate: RationalTime(value: 24, timescale: 1),
        color: settings.color,
        backgroundColor: ProjectRGBAColor(red: 0.1, green: 0.2, blue: 0.3, alpha: 1),
        layerIDs: [layerID]
    )
    let oldProject = OldPhase6ProjectDTO(
        schemaVersion: 2,
    ...
    let oldManifest = OldPhase6ManifestDTO(
        schemaVersion: 2,
        minimumReaderVersion: 2,
        projectID: projectID,
        createdByAppVersion: "6.0.0",
