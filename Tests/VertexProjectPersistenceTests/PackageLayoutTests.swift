...
private func temporaryPackage(_ name: String = UUID().uuidString) -> URL {
...
func createsRequiredDirectories() throws {
    let root = temporaryPackage()
    ...
    #expect(entries == ["Autosaves", "Bookmarks", "Journal", "Media"])
}

@Test("Canonical steady state accepts only exact root entries")
func acceptsExactSteadyStateAllowlist() throws {
    let root = temporaryPackage()
    defer { try? FileManager.default.removeItem(at: root) }
    let layout = try VertexProjectPackageLayout(root: root)
    try layout.createRequiredDirectories(fileManager: .default)
...
@Test("Legacy extension and legacy persistence entries are rejected")
func rejectsLegacyExtensionAndEntries() throws {
    ...
    for forbidden in [
        "history.json", "project.json.backup", "operations.log",
        "snapshot-current.json", "snapshot-previous.json",
        "proxies", "thumbnails", "recovery", "quarantine"
    ] {
        let root = temporaryPackage()
        defer { try? FileManager.default.removeItem(at: root) }
        let layout = try VertexProjectPackageLayout(root: root)
...
@Test("Only exact transaction temporary names are recognized")
func recognizesOnlyExactTemporaryNames() throws {
    let root = temporaryPackage()
    defer { try? FileManager.default.removeItem(at: root) }
    let layout = try VertexProjectPackageLayout(root: root)
    try layout.createRequiredDirectories(fileManager: .default)
...
@Test("Unknown canonical root entries are rejected")
func rejectsUnknownRootEntries() throws {
    let root = temporaryPackage()
    defer { try? FileManager.default.removeItem(at: root) }
    let layout = try VertexProjectPackageLayout(root: root)
    try layout.createRequiredDirectories(fileManager: .default)
    try Data().write(to: root.appendingPathComponent("extra.json"))
