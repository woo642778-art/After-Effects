...
private func makeDocument(
    ...
    layers: [ProjectLayer],
    activeCompositionID: VertexID? = compositionID,
    selectedLayerID: VertexID? = topLayerID,
    selectedMediaID: VertexID? = mediaID
) -> ProjectDocument {
    ProjectDocument(
        projectID: projectID,
        revision: 0,
...
@Test("Canonical schema 2 preserves authoritative Z-order and excludes persistence state")
func canonicalSchemaPreservesZOrderAndExcludesForbiddenFields() throws {
    ...
    for forbidden in [
        "bookmarkData", "appliedCommandIDs", "legacyRenderSettings", "renderSettings",
        "history", "undo", "redo", "forwardOperation", "inverseOperation", "operations.log"
    ] {
        #expect(!json.contains(forbidden))
    }
}

@Test("Invalid workspace convenience IDs normalize without rejecting project data")
func invalidConvenienceIDsNormalize() throws {
    let otherCompositionID = VertexID(rawValue: "60000000-0000-0000-0000-000000000099")
    let otherLayerID = VertexID(rawValue: "60000000-0000-0000-0000-000000000098")
    let otherMediaID = VertexID(rawValue: "60000000-0000-0000-0000-000000000097")
    let composition = makeComposition(layerIDs: [])
    let normalized = try makeDocument(
        compositions: [composition],
        layers: [],
        activeCompositionID: otherCompositionID,
        selectedLayerID: otherLayerID,
        selectedMediaID: otherMediaID
    ).normalized().validated()

    #expect(normalized.activeCompositionID == composition.id)
