...
func registryStartsEmpty() async throws {
    ...
    #expect(await registry.residentModelIDs.isEmpty)
}
...
func missingModelFailsClosed() async throws {
    let root = FileManager.default.temporaryDirectory
        .appendingPathComponent("vertex-ai-registry-missing-\(UUID().uuidString)", isDirectory: true)
    let registry = try AIModelRegistry(resourceRoot: root, manifest: registryManifest())
    do {
        _ = try await registry.model(for: "fixture-model")
