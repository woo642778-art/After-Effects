...
internal struct MetalGraphExecutor {
    func execute(
        ...
        commandBuffer.label = "Vertex Phase 6 Composition Graph"
        ...
        for node in plan.orderedNodes {
            ...
            case .source(let image):
                ...
                values[node.id] = MetalTextureValue(texture: texture, width: decoded.width, height: decoded.height)
            ...
            case .solidColor(let color):
                ...
                values[node.id] = MetalTextureValue(texture: texture, width: output.width, height: output.height)
        ...
        guard commandBuffer.status != .error else {
            throw RenderError.commandFailure(commandBuffer.error?.localizedDescription ?? "The composition command buffer failed.")
        }
        try await cancellationToken.throwIfCancelled()
    ...
    private func dependencyValue(
        ...
        guard let value = values[id] else {
            throw RenderError.commandFailure("A dependency texture is unavailable: \(id.rawValue).")
        }
        return value
    }
    ...
    private func encodeSolid(
        ...
        guard let encoder = commandBuffer.makeComputeCommandEncoder() else {
            throw RenderError.commandFailure("A solid-color compute encoder could not be created.")
        }
        ...
        var parameters = MetalSolidParameters(
            color: SIMD4(Float(color.red), Float(color.green), Float(color.blue), Float(color.alpha))
        )
        encoder.setBytes(&parameters, length: MemoryLayout<MetalSolidParameters>.stride, index: 0)
        dispatch(encoder: encoder, pipeline: resources.solidPipeline, width: output.width, height: output.height)
        encoder.endEncoding()
    }
    ...
    private func encodeLayer(
        ...
        for operation in operations {
            ...
            case .transform2D(let value): transform = value
            case .exposure(let stops): exposure = Float(stops)
            case .saturation(let value): saturation = Float(value)
            case .invert(let enabled): invert = enabled ? 1 : 0
            case .opacity(let value): opacity = Float(value)
            }
        }
        ...
        var parameters = MetalLayerParameters(
            dimensions: SIMD4(
                UInt32(input.width), UInt32(input.height),
                UInt32(outputWidth), UInt32(outputHeight)
            ),
            positionAnchor: SIMD4(
                Float(transform.positionX), Float(transform.positionY),
                Float(transform.anchorX), Float(transform.anchorY)
            ),
            scaleRotationOpacity: SIMD4(
                Float(transform.scaleX), Float(transform.scaleY),
                Float(transform.rotationDegrees * .pi / 180), opacity
            ),
            effects: SIMD4(exposure, saturation, invert, 0)
        )
        encoder.setBytes(&parameters, length: MemoryLayout<MetalLayerParameters>.stride, index: 0)
        dispatch(encoder: encoder, pipeline: resources.layerPipeline, width: outputWidth, height: outputHeight)
        encoder.endEncoding()
    }
    ...
    private func encodeComposite(
        ...
        var parameters = MetalCompositeParameters(
            dimensionsAndMode: SIMD4(UInt32(width), UInt32(height), blendMode.metalValue, 0)
        )
        encoder.setBytes(&parameters, length: MemoryLayout<MetalCompositeParameters>.stride, index: 0)
        dispatch(encoder: encoder, pipeline: resources.compositePipeline, width: width, height: height)
        encoder.endEncoding()
    }
    ...
    private func encodeAdjustment(
        ...
        for operation in operations {
            ...
            case .opacity, .transform, .transform2D:
                throw RenderError.invalidRequest("Adjustment nodes support exposure, saturation, and inversion only.")
            }
        }
        guard let encoder = commandBuffer.makeComputeCommandEncoder() else {
        ...
        encoder.setBytes(&parameters, length: MemoryLayout<MetalAdjustmentParameters>.stride, index: 0)
        dispatch(encoder: encoder, pipeline: resources.adjustmentPipeline, width: width, height: height)
        encoder.endEncoding()
    }
