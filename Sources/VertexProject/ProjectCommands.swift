...
public struct ProjectCommandEngine: Sendable {
    ...
    public func prepare(
        ...
        case .createComposition(let composition, let ownedLayers, let index):
            guard (0...document.compositionRegistry.count).contains(index) else {
                throw ProjectError.invalidOperation("Composition insertion index is outside the registry.")
            }
        ...
        case .removeComposition(let compositionID):
            guard document.compositionRegistry.count > 1 else {
                throw ProjectError.invalidOperation("A project must retain at least one composition.")
            }
            guard let index = document.compositionRegistry.firstIndex(where: { $0.id == compositionID }) else {
                throw ProjectError.invalidOperation("Composition does not exist.")
            }
            guard document.activeCompositionID != compositionID else {
                throw ProjectError.invalidOperation("The active composition cannot be removed.")
            }
            guard !document.layerRegistry.contains(where: { layer in
                guard layer.compositionID != compositionID else { return false }
                if case .composition(let targetID, _) = layer.source {
                    return targetID == compositionID
                }
                return false
            }) else {
                throw ProjectError.invalidOperation("A referenced composition cannot be removed.")
            }
            let composition = document.compositionRegistry[index]
            let ownedLayers = composition.layerIDs.compactMap { document.layer(id: $0) }
            guard ownedLayers.count == composition.layerIDs.count else {
                throw ProjectError.invalidOperation("Composition ownership is incomplete.")
            }
        ...
        case .duplicateComposition(let sourceID, let newCompositionID, let newLayerIDs):
            guard let sourceIndex = document.compositionRegistry.firstIndex(where: { $0.id == sourceID }) else {
                throw ProjectError.invalidOperation("Source composition does not exist.")
            }
            ...
            guard newLayerIDs.count == source.layerIDs.count,
                  Set(newLayerIDs).count == newLayerIDs.count else {
                throw ProjectError.invalidValue("Duplicated composition layer identities must match the source layer count and be unique.")
            }
            guard !newLayerIDs.contains(where: { id in document.layerRegistry.contains(where: { $0.id == id }) }) else {
                throw ProjectError.duplicateIdentity("layer")
            }
            let sourceLayers = source.layerIDs.compactMap { document.layer(id: $0) }
            guard sourceLayers.count == source.layerIDs.count else {
                throw ProjectError.invalidOperation("Source composition ownership is incomplete.")
            }
            ...
            let registryIndex = sourceIndex + 1
            forward = .duplicateComposition(
                sourceID: sourceID,
        ...
        case .renameComposition(let compositionID, let nextName):
            ...
            guard normalized != composition.name else {
                throw ProjectError.invalidOperation("Composition name is already set to the requested value.")
            }
        ...
        case .setCompositionDimensions(let compositionID, let width, let height):
            ...
            guard composition.width != width || composition.height != height else {
                throw ProjectError.invalidOperation("Composition dimensions are already set to the requested values.")
            }
        ...
        case .setCompositionDuration(let compositionID, let duration):
            ...
            guard duration != composition.duration else {
                throw ProjectError.invalidOperation("Composition duration is already set to the requested value.")
            }
        ...
        case .setCompositionFrameRate(let compositionID, let frameRate):
            ...
            guard frameRate != composition.frameRate else {
                throw ProjectError.invalidOperation("Composition frame rate is already set to the requested value.")
            }
        ...
        case .setCompositionBackground(let compositionID, let color):
            ...
            guard color != composition.backgroundColor else {
                throw ProjectError.invalidOperation("Composition background is already set to the requested value.")
            }
        ...
        case .insertLayer(let layer, let compositionID, let index):
            ...
            guard layer.compositionID == compositionID else {
                throw ProjectError.invalidValue("Inserted layer ownership does not match the requested composition.")
            }
            guard !document.layerRegistry.contains(where: { $0.id == layer.id }) else {
                throw ProjectError.duplicateIdentity("layer")
            }
            guard (0...composition.layerIDs.count).contains(index) else {
                throw ProjectError.invalidOperation("Layer insertion index is outside the composition order.")
            }
        ...
        case .removeLayer(let layerID):
            ...
            guard let composition = document.composition(id: layer.compositionID),
                  let index = composition.layerIDs.firstIndex(of: layerID) else {
                throw ProjectError.invalidOperation("Layer ownership is inconsistent.")
            }
        ...
        case .duplicateLayer(let sourceID, let duplicateID, let index):
            ...
            guard let composition = document.composition(id: source.compositionID),
                  (0...composition.layerIDs.count).contains(index) else {
                throw ProjectError.invalidOperation("Duplicate layer insertion index is outside the composition order.")
            }
            var duplicate = source
        ...
        case .renameLayer(let layerID, let nextName):
            ...
            guard normalized != layer.name else {
                throw ProjectError.invalidOperation("Layer name is already set to the requested value.")
            }
        ...
        case .reorderLayer(let compositionID, let layerID, let toIndex):
            ...
            guard layer.compositionID == compositionID,
                  let composition = document.composition(id: compositionID),
                  let beforeIndex = composition.layerIDs.firstIndex(of: layerID) else {
                throw ProjectError.invalidOperation("Layer does not belong to the requested composition.")
            }
            guard composition.layerIDs.indices.contains(toIndex) else {
                throw ProjectError.invalidOperation("Layer reorder index is outside the composition order.")
            }
            guard beforeIndex != toIndex else {
                throw ProjectError.invalidOperation("Layer is already at the requested index.")
            }
        ...
        case .setLayerEnabled(let layerID, let value):
            let layer = try editableLayer(id: layerID, in: document)
            guard layer.enabled != value else {
                throw ProjectError.invalidOperation("Layer enabled state is already set to the requested value.")
            }
            forward = .setLayerEnabled(id: layerID, before: layer.enabled, after: value)

        case .setLayerLocked(let layerID, let value):
            guard let layer = document.layer(id: layerID) else {
                throw ProjectError.invalidOperation("Layer does not exist.")
            }
            guard layer.locked != value else {
                throw ProjectError.invalidOperation("Layer locked state is already set to the requested value.")
            }
            forward = .setLayerLocked(id: layerID, before: layer.locked, after: value)

        case .setLayerSolo(let layerID, let value):
            let layer = try editableLayer(id: layerID, in: document)
            guard layer.solo != value else {
                throw ProjectError.invalidOperation("Layer solo state is already set to the requested value.")
            }
            forward = .setLayerSolo(id: layerID, before: layer.solo, after: value)

        case .setLayerTiming(let layerID, let value):
            let layer = try editableLayer(id: layerID, in: document)
            guard value != layer.timing else {
                throw ProjectError.invalidOperation("Layer timing is already set to the requested value.")
            }
            if let composition = document.composition(id: layer.compositionID) {
                _ = try value.validated(for: composition)
            }
        ...
        case .setLayerTransform(let layerID, let value):
            ...
            guard value != layer.transform else {
                throw ProjectError.invalidOperation("Layer transform is already set to the requested value.")
            }
            forward = .setLayerTransform(id: layerID, before: layer.transform, after: value)

        case .setLayerBlendMode(let layerID, let value):
            let layer = try editableLayer(id: layerID, in: document)
            guard value != layer.blendMode else {
                throw ProjectError.invalidOperation("Layer blend mode is already set to the requested value.")
            }
            forward = .setLayerBlendMode(id: layerID, before: layer.blendMode, after: value)

        case .setLayerSource(let layerID, let value):
            let layer = try editableLayer(id: layerID, in: document)
            guard value != layer.source else {
                throw ProjectError.invalidOperation("Layer source is already set to the requested value.")
            }
        ...
        case .setLayerOperations(let layerID, let value):
            ...
            guard value != layer.operations else {
                throw ProjectError.invalidOperation("Layer operations are already set to the requested value.")
            }
    ...
    private func validateNewComposition(
        ...
        against document: ProjectDocument
    ) throws {
        guard !document.compositionRegistry.contains(where: { $0.id == composition.id }) else {
            throw ProjectError.duplicateIdentity("composition")
        }
        let layerIDs = ownedLayers.map(\.id)
        guard Set(layerIDs).count == layerIDs.count,
              Set(layerIDs) == Set(composition.layerIDs),
              layerIDs.count == composition.layerIDs.count else {
            throw ProjectError.invalidValue("Owned layers must exactly match the composition layer order identities.")
        }
        guard ownedLayers.allSatisfy({ $0.compositionID == composition.id }) else {
            throw ProjectError.invalidValue("Owned layers must belong to the created composition.")
        }
        guard !layerIDs.contains(where: { id in document.layerRegistry.contains(where: { $0.id == id }) }) else {
            throw ProjectError.duplicateIdentity("layer")
        }
    }
    ...
    private func editableLayer(id: VertexID, in document: ProjectDocument) throws -> ProjectLayer {
        ...
        guard !layer.locked else {
            throw ProjectError.invalidOperation("Locked layers must be explicitly unlocked before editing.")
        }
        return layer
    }
    ...
    private func apply(_ mutation: ProjectMutation, to document: inout ProjectDocument) throws {
        ...
        case .createComposition(let composition, let ownedLayers, let registryIndex):
            guard (0...document.compositionRegistry.count).contains(registryIndex),
                  !document.compositionRegistry.contains(where: { $0.id == composition.id }) else {
                throw ProjectError.invalidOperation("Composition creation precondition did not match.")
            }
        ...
        case .removeComposition(let composition, let ownedLayers, let registryIndex):
            guard document.compositionRegistry.indices.contains(registryIndex),
                  document.compositionRegistry[registryIndex] == composition else {
                throw ProjectError.invalidOperation("Composition removal precondition did not match.")
            }
            guard document.activeCompositionID != composition.id else {
                throw ProjectError.invalidOperation("The active composition cannot be removed.")
            }
            guard !document.layerRegistry.contains(where: { layer in
            ...
            guard actualLayers == ownedLayers else {
                throw ProjectError.invalidOperation("Owned-layer removal precondition did not match.")
            }
            ...
            if let selectedLayerID = document.selectedLayerID, ownedIDs.contains(selectedLayerID) {
        ...
        case .duplicateComposition(let sourceID, let composition, let layers, let registryIndex):
            guard document.composition(id: sourceID) != nil else {
                throw ProjectError.invalidOperation("Duplicate source composition no longer exists.")
            }
            try validateNewComposition(composition, ownedLayers: layers, against: document)
            guard (0...document.compositionRegistry.count).contains(registryIndex) else {
                throw ProjectError.invalidOperation("Duplicate composition insertion index is outside the registry.")
            }
            document.compositionRegistry.insert(composition, at: registryIndex)
        ...
        case .removeDuplicatedComposition(let sourceID, let composition, let layers, let registryIndex):
            guard document.composition(id: sourceID) != nil,
                  document.compositionRegistry.indices.contains(registryIndex),
                  document.compositionRegistry[registryIndex] == composition else {
                throw ProjectError.invalidOperation("Duplicated composition removal precondition did not match.")
            }
            let actualLayers = composition.layerIDs.compactMap { document.layer(id: $0) }
            guard actualLayers == layers else {
                throw ProjectError.invalidOperation("Duplicated composition layers no longer match.")
            }
            let layerIDs = Set(layers.map(\.id))
            document.layerRegistry.removeAll { layerIDs.contains($0.id) }
            document.compositionRegistry.remove(at: registryIndex)
            if let selectedLayerID = document.selectedLayerID, layerIDs.contains(selectedLayerID) {
                document.selectedLayerID = nil
            }

        case .renameComposition(let id, let before, let after):
            guard let index = document.compositionRegistry.firstIndex(where: { $0.id == id }),
                  document.compositionRegistry[index].name == before else {
        ...
        case .setCompositionDimensions(let id, let beforeWidth, let beforeHeight, let afterWidth, let afterHeight):
            guard let index = document.compositionRegistry.firstIndex(where: { $0.id == id }),
                  document.compositionRegistry[index].width == beforeWidth,
        ...
        case .setCompositionDuration(let id, let before, let after):
            guard let index = document.compositionRegistry.firstIndex(where: { $0.id == id }),
                  document.compositionRegistry[index].duration == before else {
        ...
        case .setCompositionFrameRate(let id, let before, let after):
            guard let index = document.compositionRegistry.firstIndex(where: { $0.id == id }),
                  document.compositionRegistry[index].frameRate == before else {
        ...
        case .setCompositionBackground(let id, let before, let after):
            guard let index = document.compositionRegistry.firstIndex(where: { $0.id == id }),
                  document.compositionRegistry[index].backgroundColor == before else {
        ...
        case .insertLayer(let layer, let compositionID, let index):
            guard let compositionIndex = document.compositionRegistry.firstIndex(where: { $0.id == compositionID }),
                  (0...document.compositionRegistry[compositionIndex].layerIDs.count).contains(index),
                  layer.compositionID == compositionID,
        ...
        case .removeLayer(let layer, let compositionID, let index):
            guard let compositionIndex = document.compositionRegistry.firstIndex(where: { $0.id == compositionID }),
                  document.compositionRegistry[compositionIndex].layerIDs.indices.contains(index),
                  document.compositionRegistry[compositionIndex].layerIDs[index] == layer.id,
                  let registryIndex = document.layerRegistry.firstIndex(of: layer) else {
        ...
        case .duplicateLayer(let sourceLayerID, let duplicate, let compositionID, let index):
            guard document.layer(id: sourceLayerID) != nil,
                  let compositionIndex = document.compositionRegistry.firstIndex(where: { $0.id == compositionID }),
                  (0...document.compositionRegistry[compositionIndex].layerIDs.count).contains(index),
                  duplicate.compositionID == compositionID,
                  !document.layerRegistry.contains(where: { $0.id == duplicate.id }) else {
                throw ProjectError.invalidOperation("Layer duplication precondition did not match.")
            }
        ...
        case .removeDuplicatedLayer(let sourceLayerID, let duplicate, let compositionID, let index):
            guard document.layer(id: sourceLayerID) != nil,
                  let compositionIndex = document.compositionRegistry.firstIndex(where: { $0.id == compositionID }),
                  document.compositionRegistry[compositionIndex].layerIDs.indices.contains(index),
                  document.compositionRegistry[compositionIndex].layerIDs[index] == duplicate.id,
                  let registryIndex = document.layerRegistry.firstIndex(of: duplicate) else {
                throw ProjectError.invalidOperation("Duplicated-layer removal precondition did not match.")
            }
            document.compositionRegistry[compositionIndex].layerIDs.remove(at: index)
            document.layerRegistry.remove(at: registryIndex)
            if document.selectedLayerID == duplicate.id {
                document.selectedLayerID = nil
            }

        case .renameLayer(let id, let before, let after):
            guard let index = document.layerRegistry.firstIndex(where: { $0.id == id }),
                  document.layerRegistry[index].name == before else {
        ...
        case .reorderLayer(let compositionID, let layerID, let beforeIndex, let afterIndex):
            guard let compositionIndex = document.compositionRegistry.firstIndex(where: { $0.id == compositionID }) else {
                throw ProjectError.invalidOperation("Composition does not exist.")
            }
            var order = document.compositionRegistry[compositionIndex].layerIDs
            guard order.indices.contains(beforeIndex), order[beforeIndex] == layerID else {
                throw ProjectError.invalidOperation("Layer reorder precondition did not match.")
            }
            let removed = order.remove(at: beforeIndex)
            guard (0...order.count).contains(afterIndex) else {
                throw ProjectError.invalidOperation("Layer reorder destination is outside the composition order.")
            }
    ...
    private func setLayer(
        ...
        matching predicate: (ProjectLayer) -> Bool,
        mutate: (inout ProjectLayer) -> Void
    ) throws {
        guard let index = document.layerRegistry.firstIndex(where: { $0.id == id }),
              predicate(document.layerRegistry[index]) else {
            throw ProjectError.invalidOperation("Layer property precondition did not match.")
        }
