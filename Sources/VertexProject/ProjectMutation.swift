...
public enum ProjectMutation: Codable, Equatable, Sendable {
    ...
    case registerBakedAIEffect(ProjectAIEffectBakeRegistration, previousSelectedLayerID: VertexID?)
    ...
    case setLayerMasks(layerID: VertexID, before: [ProjectMask], after: [ProjectMask])
    ...
    public var inverse: ProjectMutation {
        ...
        case .registerBakedAIEffect(let registration, let previous): .removeBakedAIEffect(registration, restoreSelectedLayerID: previous)
        ...
        case .setLayerMasks(let id, let before, let after): .setLayerMasks(layerID: id, before: after, after: before)
