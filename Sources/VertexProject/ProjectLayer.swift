...
public struct CameraLayerSettings: Codable, Equatable, Sendable {
    ...
    public func validated() throws -> Self {
        let values = [
            focalLengthMillimeters, nearClip, farClip,
            positionX, positionY, positionZ,
            pointOfInterestX, pointOfInterestY, pointOfInterestZ
        ]
        guard values.allSatisfy(\.isFinite), focalLengthMillimeters > 0,
              nearClip > 0, farClip > nearClip else {
...
public struct LightLayerSettings: Codable, Equatable, Sendable {
    ...
    public func validated() throws -> Self {
        let values = [
            intensity, positionX, positionY, positionZ,
            directionX, directionY, directionZ,
            coneAngleDegrees, coneFeather
        ]
        guard values.allSatisfy(\.isFinite), intensity >= 0,
              (0...180).contains(coneAngleDegrees), (0...1).contains(coneFeather) else {
...
public struct LayerTransform: Codable, Equatable, Sendable {
    ...
    public func validated() throws -> Self {
        ...
        guard values.allSatisfy(\.isFinite), scaleX > 0, scaleY > 0,
    ...
    public func validated() throws -> Self {
        ...
        case .exposure(let stops):
            guard stops.isFinite else {
                throw ProjectError.invalidValue("Layer exposure must be finite.")
            }
        case .saturation(let value):
            guard value.isFinite, value >= 0 else {
                throw ProjectError.invalidValue("Layer saturation must be finite and nonnegative.")
            }
        case .invert:
            break
        }
...
public struct ProjectLayer: Codable, Equatable, Sendable, Identifiable {
    ...
    public func validated(in document: ProjectDocument) throws -> Self {
        ...
        case .media(let mediaID, let sourceStartTime):
            guard sourceStartTime >= .zero else {
                throw ProjectError.invalidValue("Media source start time must be nonnegative.")
            }
        ...
        case .composition(let targetID, let sourceStartTime):
            guard sourceStartTime >= .zero else {
                throw ProjectError.invalidValue("Nested source start time must be nonnegative.")
            }
        ...
        case .adjustment:
            guard blendMode == .normal else {
                throw ProjectError.invalidValue("Adjustment layers use Normal blend mode in schema 2.")
            }
        case .null, .guide:
            guard blendMode == .normal, operations.isEmpty else {
                throw ProjectError.invalidValue("Model-only layers use Normal blend mode and no pixel operations.")
            }
        case .camera(let settings):
            _ = try settings.validated()
            guard blendMode == .normal, operations.isEmpty else {
                throw ProjectError.invalidValue("Camera layers use Normal blend mode and no pixel operations.")
            }
        case .light(let settings):
            _ = try settings.validated()
            guard blendMode == .normal, operations.isEmpty else {
                throw ProjectError.invalidValue("Light layers use Normal blend mode and no pixel operations.")
            }
        }
        return self
    }
