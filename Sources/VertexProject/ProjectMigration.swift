...
public struct ProjectCompatibilityInfo: Codable, Equatable, Sendable {
    ...
    public init(schemaVersion: Int, minimumReaderVersion: Int, projectID: String?, projectName: String?, lastSavedByAppVersion: String?) {
...
public struct ProjectMigrationStepResult: Equatable, Sendable {
    ...
    public init(data: Data, report: ProjectMigrationReport) { self.data = data; self.report = report }
}

public struct ProjectMigrationResult: Equatable, Sendable {
    public var data: Data
    public var reports: [ProjectMigrationReport]
    public init(data: Data, reports: [ProjectMigrationReport]) { self.data = data; self.reports = reports }
}
...
public struct ProjectMigrationRegistry: Sendable {
    public static let current = ProjectMigrationRegistry(migrators: [Schema1To2Migrator()])
    private let migrators: [any ProjectMigrator]

    public init(migrators: [any ProjectMigrator]) {
        self.migrators = migrators.sorted {
            if $0.sourceVersion == $1.sourceVersion { return $0.destinationVersion < $1.destinationVersion }
    ...
    public func inspect(_ data: Data) throws -> ProjectCompatibilityInfo {
        ...
        catch { throw ProjectError.decodingFailure("Project compatibility header could not be read: \(error.localizedDescription)") }
        guard let root = object as? [String: Any], let schemaVersion = integer(root["schemaVersion"]) else {
        ...
        return ProjectCompatibilityInfo(schemaVersion: schemaVersion, minimumReaderVersion: minimumReaderVersion, projectID: root["projectID"] as? String, projectName: metadata?["name"] as? String, lastSavedByAppVersion: metadata?["lastSavedByAppVersion"] as? String)
    }

    public func migrate(_ data: Data, from sourceVersion: Int, to destinationVersion: Int) throws -> ProjectMigrationResult {
        guard sourceVersion <= destinationVersion else { throw ProjectError.migrationFailure("Project migrations cannot run backwards.") }
        if sourceVersion == destinationVersion { return ProjectMigrationResult(data: data, reports: []) }
        var currentData = data
        var version = sourceVersion
        var reports: [ProjectMigrationReport] = []
        while version < destinationVersion {
            guard let migrator = migrators.first(where: { $0.sourceVersion == version && $0.destinationVersion == version + 1 }) else {
            ...
            guard result.report.sourceVersion == version, result.report.destinationVersion == version + 1 else { throw ProjectError.migrationFailure("Migrator report did not match its declared version transition.") }
            let inspected = try inspect(result.data)
            guard inspected.schemaVersion == version + 1 else { throw ProjectError.migrationFailure("Migrated data did not declare schema \(version + 1).") }
