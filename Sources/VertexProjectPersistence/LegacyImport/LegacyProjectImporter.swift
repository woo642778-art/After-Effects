...
public struct LegacyProjectImporter: Sendable {
    ...
    public func convert(
        ...
        guard !fileManager.fileExists(atPath: destination.path) else {
            throw ProjectPersistenceError.legacyImportIncomplete(
                stage: "destination already exists"
            )
        }
        ...
        do {
            ...
            for (mediaID, sourceMediaURL) in context.embeddedSourceURLs {
                guard let index = document.mediaRegistry.firstIndex(where: { $0.id == mediaID }) else {
                    throw ProjectPersistenceError.legacyImportIncomplete(
                        stage: "embedded media ID missing from canonical registry"
                    )
                }
                document.mediaRegistry[index] = try embeddedStore.embed(
                    reference: document.mediaRegistry[index],
            ...
            guard digestAfter == context.sourceDigest else {
                throw ProjectPersistenceError.legacyImportIncomplete(
                    stage: "legacy source changed during conversion"
                )
            }
            ...
            guard case .opened(let snapshot) = finalResult else {
                throw ProjectPersistenceError.legacyImportIncomplete(
                    stage: "converted package requires an unexpected pending decision"
                )
            }
            ...
            throw ProjectPersistenceError.legacyImportIncomplete(
                stage: "conversion: \(error.localizedDescription)"
            )
        }
    }
    ...
    private func loadContext(sourceURL: URL) throws -> LegacyImportContext {
        ...
        guard fileManager.fileExists(atPath: layout.projectURL.path),
            ...
            throw ProjectPersistenceError.legacyImportIncomplete(
                stage: "legacy project or manifest missing"
            )
        }
        ...
        do {
            ...
            throw ProjectPersistenceError.legacyImportIncomplete(
                stage: "legacy package read: \(error.localizedDescription)"
            )
        }
    ...
    private func normalizedDocumentForStaging(
        ...
        for index in document.mediaRegistry.indices {
            ...
            document.mediaRegistry[index].locator = MediaLocator(
                relativeHint: document.mediaRegistry[index].locator.relativeHint,
                embeddedPath: nil
            )
    ...
    private func validatedDestination(_ url: URL) throws -> URL {
        ...
        guard standardized.pathExtension.lowercased() == VertexProjectPackageLayout.requiredExtension else {
            throw ProjectPersistenceError.unsupportedPackageExtension(
                found: standardized.pathExtension.lowercased()
            )
        }
        return standardized
    }
